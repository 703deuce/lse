/**
 * Capture Maps Campaigns list preview for mockup verification.
 */
...
const OUT = "/opt/cursor/artifacts/screenshots/mockup-verify/maps-campaigns-list";
...
async function main() {
  ...
  const url = `${BASE}/dev/maps-campaigns-list-preview`;
  console.log("Opening", url);
  await page.goto(url, { waitUntil: "networkidle", timeout: 120000 });
  await page.waitForSelector("text=Maps Campaigns", { timeout: 60000 });
  await page.waitForTimeout(1200);

  const overview = path.join(OUT, "01-list-overview.png");
  await page.screenshot({ path: overview, fullPage: false });
  console.log("wrote", overview, md5(overview));

  const full = path.join(OUT, "01-list-full.png");
  ...
  while (y < metrics.scrollHeight - 20 && i < 6) {
    ...
    const file = path.join(
      OUT,
      `01-list-scroll-${String(i + 1).padStart(2, "0")}.png`
    );
