...
export function GoogleReviewQrSeoLanding({ embed = false }: { embed?: boolean }) {
  if (embed) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] px-3 py-4 sm:px-6">
  ...
  return (
    <div className="min-h-screen bg-white text-[#334155]">
      <header className="sticky top-0 z-40 border-b border-[#E2E8F0] bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          <a href={MARKETING} className="text-sm font-bold text-[#0B1220]">
            Local SEO <span className="text-[#137752]">Express</span>
          </a>
          <nav className="hidden items-center gap-5 text-sm font-semibold text-[#475569] md:flex">
            <a href={`${MARKETING}/free-tools/`} className="hover:text-[#137752]">
              Free Tools
            </a>
            <a href="#faq" className="hover:text-[#137752]">
              FAQ
            </a>
            <Link href="/sign-in" className="hover:text-[#137752]">
              Log In
            </Link>
            <Link
              href="/sign-up"
              className="inline-flex h-10 items-center rounded-lg bg-[#137752] px-4 text-white"
            >
              Start for Free
            </Link>
          </nav>
          <Link href="/sign-up" className="md:hidden text-sm font-bold text-[#137752]">
            Start free
          </Link>
        </div>
      </header>
      ...
      <main>
        {/* Hero + tool */}
        <section className="border-b border-[#E2E8F0] bg-[radial-gradient(ellipse_at_top,_#ECFDF5_0%,_#ffffff_55%)]">
          <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:py-14">
            <p className="text-center text-[11px] font-bold uppercase tracking-[0.16em] text-[#137752]">
              100% free Google Review QR Code Generator
            </p>
            <h1 className="mt-3 text-center text-4xl font-extrabold tracking-tight text-[#0B1220] sm:text-5xl lg:text-6xl">
              Free Google Review QR Code Generator
            </h1>
            <div className="mx-auto mt-5 max-w-3xl space-y-4 text-center text-base leading-7 text-[#475569]">
              <p>
              ...
              <p>
                ...
                searching. No friction. Just scan, rate, and review. Whether you need a Google
                review poster for your front desk or a printable QR code for Google reviews on
                invoices and trucks, this free tool gets you there fast.
              </p>
              <p>
                Use it to create a Google Review QR Code free, download a high-quality image, and
                print it anywhere. When you&apos;re ready, create a free Local SEO Express account to
                track Google Review QR Code scans, compare placements, and manage review campaigns
                alongside SMS and email requests.
              </p>
            </div>
            <ul className="mx-auto mt-5 flex max-w-xl flex-wrap justify-center gap-x-5 gap-y-2 text-sm font-semibold text-[#0B1220]">
              {["Free for Everyone", "No Account Required", "High-Quality Downloads"].map((t) => (
                <li key={t} className="inline-flex items-center gap-1.5">
                  <Check className="h-4 w-4 text-[#137752]" />
                  {t}
                </li>
              ))}
            </ul>

            <div id="generator" className="mt-10 scroll-mt-24">
              <PublicQrGenerator embedded />
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto grid max-w-6xl gap-4 px-4 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
            {[
              ...
              <div key={item.title} className="rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-5">
                <item.icon className="h-6 w-6 text-[#137752]" />
        ...
        <ContentSection title="How to Create a Google Review QR Code">
          <ol className="mt-2 space-y-3">
            {[
              "Enter your business or Google review link.",
              "Customize your poster.",
              "Generate your QR code.",
              "Download and print.",
              "Display it where customers will see it.",
            ].map((step, i) => (
              <li key={step} className="flex gap-3 text-base">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#137752] text-sm font-bold text-white">
                  {i + 1}
                </span>
                <span>{step}</span>
              </li>
          ...
          <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                { icon: Car, title: "Leave-behinds", body: "Send technicians out with a printable QR card." },
              ].map((item) => (
                <div
                  key={item.title}
                  className="flex gap-3 rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-4"
                >
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#ECFDF5] text-[#137752]">
        ...
        {/* Comparison — stronger table from mock #1 */}
        <section className="border-b border-[#E2E8F0] bg-[#F8FAFC]">
          <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
            <h2 className="text-3xl font-extrabold tracking-tight text-[#0B1220]">
            ...
            <div className="mt-8 overflow-x-auto rounded-2xl border border-[#E2E8F0] bg-white">
              <table className="w-full min-w-[560px] text-left text-sm">
                <thead className="bg-[#0B1220] text-white">
                  <tr>
                    <th className="px-5 py-4 font-semibold">Feature</th>
                    <th className="px-5 py-4 font-semibold">Basic Google QR</th>
                    <th className="px-5 py-4 font-semibold">Local SEO Express</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={String(label)} className="border-t border-[#E2E8F0]">
                      <td className="px-5 py-3.5 font-medium text-[#0B1220]">{label}</td>
                      <td className="px-5 py-3.5">
                        {basic ? (
                          <Check className="h-5 w-5 text-[#137752]" />
                        ) : (
        ...
        <ContentSection title="Can You Track a Google Review QR Code?">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              >
                <card.icon className="mx-auto h-8 w-8 text-[#137752]" />
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <div key={item.title} className="rounded-2xl border border-[#E2E8F0] bg-white p-5">
                  <item.icon className="h-5 w-5 text-[#137752]" />
        ...
        <section id="faq" className="scroll-mt-24 border-b border-[#E2E8F0] bg-white">
          <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
            <h2 className="text-3xl font-extrabold tracking-tight text-[#0B1220]">
            ...
            <div className="mt-8 space-y-3">
              {FAQS.map((faq) => (
                <details
                  key={faq.q}
                  className="group rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4"
                >
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              {[
                ...
                <a
                  key={tool.title}
                  href={tool.href}
                  className="rounded-2xl border border-[#E2E8F0] bg-white p-5 transition hover:border-[#137752]"
                >
        ...
        <section className="bg-[#0B1220] text-white">
          <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-6 px-4 py-14 sm:flex-row sm:items-center sm:px-6">
            ...
            <a
              href="#generator"
              className="inline-flex h-12 items-center justify-center rounded-xl bg-[#137752] px-6 text-sm font-bold text-white"
            >
      ...
      <footer className="border-t border-[#E2E8F0] bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-8 text-sm text-[#64748B] sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <a href={`${MARKETING}/privacy-policy/`} className="hover:text-[#137752]">
              Privacy
            </a>
            <a href={`${MARKETING}/terms-of-service/`} className="hover:text-[#137752]">
              Terms
            </a>
          </div>
        </div>
...
function ContentSection({
  ...
  return (
    <section className={cn("border-b border-[#E2E8F0] bg-white", className)}>
