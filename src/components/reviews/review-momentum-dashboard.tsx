        ...
        <>
              ...
              <table className="min-w-full text-left text-xs">
                <thead className="bg-zinc-50 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
                  <tr>
                    ...
                    <th className="px-3 py-3">90D</th>
                    <th className="px-3 py-3">Avg / Wk</th>
                    <th className="px-3 py-3">Last Review</th>
                    <th className="px-3 py-3">Momentum</th>
                    <th className="px-3 py-3">Gap (30D)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50">
                  {displayEntities.map((e) => {
                    ...
                    const velocityAvailable = isTarget
                      ...
                      : true;
                    return (
                      <tr key={e.id} className="transition-colors hover:bg-zinc-50/80">
                        <td className="px-4 py-3">
                          ...
                          {isTarget && (
                            <div className="mt-1 flex flex-wrap gap-1">
                              ...
                              >
                                {velocityAvailable ? "Velocity available" : "Velocity unavailable"}
                              </span>
                            </div>
                          )}
                        </td>
                        ...
                        <td
                          className={cn(
                            ...
                            isTarget && velocityAvailable ? "text-emerald-700" : "text-zinc-700"
                          )}
                        >
                        ...
                        <td
                          className={cn(
                            "px-3 py-3",
                            isTarget && velocityAvailable ? "font-semibold text-emerald-700" : "text-zinc-700"
                          )}
                        >
                        ...
                        <td className="px-3 py-3 text-zinc-500">
                          {velocityAvailable && e.days_since_last_review != null
                            ? `${e.days_since_last_review}d ago`
                            : "—"}
                        </td>
                        <td className="px-3 py-3">
                          {velocityAvailable ? (
                        ...
                        <td className="px-3 py-3 font-medium text-zinc-700">
          ...
          {targetVelocityAvailable && (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              <MiniChartCard title="30-Day Review Velocity">
          ...
          {(data.run.ai_summary || data.tasks.length > 0) && (
            <div className="grid gap-4 lg:grid-cols-12 lg:items-start">
              {data.run.ai_summary && (
                <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-5 lg:col-span-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <h2 className="text-sm font-semibold text-zinc-900">What this means</h2>
                      <p className="mt-2 text-sm leading-relaxed text-zinc-700">{data.run.ai_summary}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {data.tasks.length > 0 && (
                <div className={cn("min-w-0", data.run.ai_summary ? "lg:col-span-8" : "lg:col-span-12")}>
                  <h2 className="mb-3 text-sm font-semibold text-zinc-900">Suggested tasks</h2>
                  ...
                  {data.tasks.map((t, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex h-full min-h-[120px] flex-col rounded-xl border border-zinc-200 bg-white p-3.5 text-left shadow-sm transition hover:border-zinc-300 hover:shadow",
                      ...
                      >
                        ...
                        <p
                          className={cn(
                            "mt-2.5 text-xs font-semibold leading-snug text-zinc-900",
                            t.status === "done" && "line-through"
                          )}
                        >
                          {t.title}
                        </p>
                        <p className="mt-1 line-clamp-2 flex-1 text-[11px] leading-relaxed text-zinc-500">
                          {t.description}
                        </p>
...
function MiniChartCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className={cn(momentumCardClass, "p-3")}>
      <h3 className="mb-2.5 text-[11px] font-semibold leading-snug text-zinc-800">{title}</h3>
      {children}
    </div>
  );
}
