...
function statusTag(status: string) {
  ...
  return (
    <span className={cn("rounded-full px-2 py-0.5 text-xs font-semibold", styles[status] ?? "bg-zinc-100 text-zinc-600")}>
...
function opportunityTag(level: string) {
  ...
  return (
    <span className={cn("rounded-full px-2 py-0.5 text-xs font-semibold capitalize", styles[level] ?? "")}>
      {level}
    </span>
  );
}
    ...
    <div className="space-y-6">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <GaCard className="!p-4">
          <p className="text-xs font-medium text-zinc-500">Service Coverage Score</p>
          <p className="mt-2 text-3xl font-bold text-zinc-900">
            {serviceCoverage.score}
            <span className="text-sm font-medium text-zinc-400">/100</span>
          </p>
          <p className="mt-0.5 text-xs font-semibold text-amber-600">Needs Improvement</p>
          <div className="mt-3">
            <Sparkline data={[serviceCoverage.score - 6, serviceCoverage.score - 3, serviceCoverage.score]} color="#059669" />
          </div>
        </GaCard>
        <GaCard className="!p-4">
          <p className="text-xs font-medium text-zinc-500">Local Coverage Score</p>
          <p className="mt-2 text-3xl font-bold text-zinc-900">
            {localCoverage.score}
            <span className="text-sm font-medium text-zinc-400">/100</span>
          </p>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
        <GaCard className="!p-0 overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-zinc-900">Service Coverage</p>
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50/80 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">
                  ...
                  <th className="px-4 py-3">On GBP</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {serviceRows.map((row) => (
                  <tr key={row.service} className="hover:bg-zinc-50/50">
                    <td className="px-4 py-3 font-medium text-zinc-900">{row.service}</td>
                    ...
                    <td className="px-4 py-3">{(row.onYourGbp ?? row.gbpListed) ? "Yes" : "No"}</td>
                    ...
                    <td className="px-4 py-3">{statusTag(row.status)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {keywordRows.length > 0 && (
            <p className="border-t border-zinc-100 px-5 py-2 text-xs text-zinc-500">
          ...
          {serviceCoverage.rows.length > 8 && (
            <div className="border-t border-zinc-100 px-5 py-3 text-center">
              <button type="button" className="text-sm font-medium text-zinc-600">
        ...
        <GaCard>
          <p className="text-sm font-semibold text-zinc-900">Service Coverage Insights</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
        <GaCard className="!p-0 overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-zinc-900">Local Coverage</p>
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50/80 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">
                  ...
                  <th className="px-4 py-3">Mentioned on site</th>
                  <th className="px-4 py-3">Competitors</th>
                  <th className="px-4 py-3">Opportunity</th>
                  <th className="px-4 py-3">Status</th>
                </tr>
              ...
              <tbody className="divide-y divide-zinc-100">
                {localRows.map((row) => (
                  <tr key={row.area} className="hover:bg-zinc-50/50">
                    ...
                    <td className="px-4 py-3 tabular-nums">{row.competitorCount}/20</td>
                    <td className="px-4 py-3">{opportunityTag(row.opportunity)}</td>
                    <td className="px-4 py-3">{statusTag(row.status)}</td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-4">
          <GaCard>
            <p className="text-sm font-semibold text-zinc-900">Market Coverage Map</p>
            <div className="mt-3 flex h-36 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-100 via-amber-50 to-red-50">
          ...
          <GaCard>
            <p className="text-sm font-semibold text-zinc-900">Local Coverage Insights</p>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <GaCard className="!p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-zinc-900">Missing service pages</p>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {missingServices.slice(0, 2).map((s) => (
                    <span key={s.service} className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-700">
                      {s.service}
                    </span>
                  ))}
                </div>
          ...
          <GaCard className="!p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-zinc-900">Category gaps from top 20</p>
                <p className="mt-1 text-sm text-zinc-500">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {(serviceCoverage.categoryGap.categoryAlignment?.recommendations ?? [])
                    .slice(0, 2)
                    .map((r) => (
                      <span key={r.category} className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-700">
                        {r.category}
                      </span>
                    ))}
                </div>
