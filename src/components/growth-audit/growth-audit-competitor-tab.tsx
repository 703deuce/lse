...
export function GrowthAuditCompetitorTab({
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 lg:grid-cols-2">
        ...
        <GaCard className="!p-0 overflow-hidden">
          <div className="border-b border-zinc-100 px-3.5 py-2.5">
            <p className="text-[13px] font-semibold text-zinc-900">How you stack up</p>
          </div>
      ...
      <GaCard>
        <p className="text-[13px] font-semibold text-zinc-900">Why competitors beat you</p>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <GaCard>
          <p className="text-[13px] font-semibold text-zinc-900">Your biggest gaps</p>
          <p className="mt-0.5 text-[11px] text-zinc-500">Visible differences vs top competitors in Maps results.</p>
          <ul className="mt-3 space-y-2">
            {result.yourGaps.length > 0 ? (
              result.yourGaps.map((gap) => (
                <li key={gap} className="flex items-start gap-2 text-[13px] text-zinc-700">
                ...
                return (
                  <li key={gap.label} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13px] font-medium text-zinc-900">{gap.label}</p>
                      <p className="text-[11px] text-zinc-500">
                        You: {gap.you} · Top 3 avg: {gap.top}
                      </p>
                    </div>
                    <span className="shrink-0 text-[13px] font-bold tabular-nums text-red-600">{diff}</span>
                  </li>
                );
              })
            ) : (
              <li className="text-[13px] text-zinc-500">No major gaps detected vs top competitors.</li>
            )}
          </ul>
          <div className="mt-3">
        ...
        <GaCard className="!p-0 overflow-hidden">
          <div className="border-b border-zinc-100 px-3.5 py-2.5">
            <p className="text-[13px] font-semibold text-zinc-900">Competitor leaderboard</p>
            <p className="text-[11px] text-zinc-500">Top 3 from Maps results by review volume.</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-[12px]">
              <thead>
